  ...
  if (empty($_POST['name']) || empty($_POST['email']) || empty($_POST['phone']) || empty($_POST['address']) ||empty($_POST['password']) || strlen($_POST['password']) < 4 )
  {
    ...
    if (!empty($_POST['role'])) {
      ...
      if ($user) {
        ...
        $stmt = $pdo->prepare("INSERT INTO users(name,password,email,address,phone,role) VALUES(:name,:password,:email,:address,:phone,:role)");
        $result = $stmt->execute(
          array(
            ':name' => $name,
            ':password' => $password,
            ...
            <div class="card">
              <div class="card-body">
                <form class="" action="user_add.php" method="post">
                  ...
                  <div class="form-group">
                     <label for="phone">Address</label><p style="color:red;"><?php echo empty($addressError) ? '' : '*'.$addressError; ?></p>
                     <input type="text" class="form-control" name="address">
                  </div>
                  <div class="form-group">
                     <label for="phone">Phone Number</label><p style="color:red;"><?php echo empty($phoneError) ? '' : '*'.$phoneError; ?></p>
